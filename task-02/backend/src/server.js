...
app.get('/health', (req, res) => res.json({ ok: true, service: 'task-02-ecommerce-backend' }));
=======
...
>>>>>>> c8e52bbd3c97e09a7f2cfafcb9e9f9787748df3b
app.get('/api/health', (req, res) => res.json({ ok: true, service: 'task-02-ecommerce-backend' }));
...
if (fs.existsSync(publicDir)) {
  ...
  app.get('/', (req, res) =>
    res.json({
      ok: true,
      service: 'task-02-ecommerce-backend',
      health: '/api/health'
    })
  );
}
...
async function start() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Task-02 E-Commerce backend listening on 0.0.0.0:${PORT}`);
  });
...
const PORT = process.env.PORT || 4002;
app.listen(PORT, () => {
  console.log(`Task-02 E-Commerce backend running on port ${PORT}`);
  startReservationExpiryJob();
});
>>>>>>> c8e52bbd3c97e09a7f2cfafcb9e9f9787748df3b
